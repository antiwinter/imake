...
export class Logger {
    ...
    log(message, resobj) {
        ...
        if (resobj) {
            ...
            const baseLength = chalk.stripColor(baseLog).length + this.tokenColor(tokens).length + this.costColor(cost).length;
            ...
            console.log(`${baseLog} ${dots} [${this.tokenColor(tokens)}] ${this.costColor(cost)}`);
        } else {
            console.log(baseLog);
        }
    }
