...
async function improveDocs(resumes, ignores) {
  ...
  const improvePrompts = markdownFiles.map((file) => {
    ...
    return openAI.query(`You're going to improve the content of ${file}
        according to the original design docs. improve the clarity and detail of the documentation,
        ensuring logical consistency and completeness.
  ...
  const summaryContent = improvements
    .map(
      (imp, index) =>
        `## ${markdownFiles[index]}\nOriginal Tokens: ${imp.uploadedTokenCount}k, Improved Tokens: ${imp.generatedTokenCount}k\n`
    )
    .join("\n");
